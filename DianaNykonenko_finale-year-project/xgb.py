...
def objective(trial):                                                                                      # optuna optimiser to find the best hyperparameters
    ...
    params = {
        ...
        "n_estimators": trial.suggest_int("n_estimators", 140, 170),  
        "max_depth": trial.suggest_int("max_depth", 6, 8),  
        "learning_rate": trial.suggest_float("learning_rate", 0.009, 0.015, log=True), 
        "subsample": trial.suggest_float("subsample", 0.68, 0.75), 
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.84, 0.90)  
    }

    model = xgb.XGBRegressor(**params)                                                                     # model base 
...
best_model = xgb.XGBRegressor(objective="reg:squarederror", **study.best_params)
